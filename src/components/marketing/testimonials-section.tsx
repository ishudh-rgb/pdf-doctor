...
const TESTIMONIALS: Testimonial[] = [
  { name: "Priya Sharma", role: "Freelance Designer", country: "India", flag: "🇮🇳", rating: 5, text: "OnlyMyPDF saved me hours of work. The merge and compress tools are blazing fast, and the quality is perfect every time.", avatar: "PS", accent: "from-pink-500 to-rose-600" },
  { name: "James Wilson", role: "Marketing Manager", country: "USA", flag: "🇺🇸", rating: 5, text: "Switched from Adobe Acrobat. OnlyMyPDF does everything I need for free — convert, edit, sign. The UI is clean and intuitive.", avatar: "JW", accent: "from-blue-500 to-indigo-600" },
  { name: "Aisha Mohammed", role: "University Student", country: "UAE", flag: "🇦🇪", rating: 5, text: "I use OnlyMyPDF daily for my assignments. Converting Word to PDF and merging files is so easy. Best free PDF tool online!", avatar: "AM", accent: "from-emerald-500 to-teal-600" },
  ...
  { name: "Liam O'Brien", role: "Startup Founder", country: "Ireland", flag: "🇮🇪", rating: 5, text: "We use OnlyMyPDF across our entire team. The batch processing and no file size limits are game-changers for us.", avatar: "LO", accent: "from-green-500 to-emerald-600" },
  ...
  { name: "Maria Santos", role: "HR Manager", country: "Brazil", flag: "🇧🇷", rating: 5, text: "We process hundreds of employee documents weekly. OnlyMyPDF handles large files without breaking a sweat. Love the speed!", avatar: "MS", accent: "from-yellow-500 to-amber-600" },
  ...
  { name: "Raj Patel", role: "CA Student", country: "India", flag: "🇮🇳", rating: 5, text: "OnlyMyPDF's Excel to PDF and PDF to Excel tools are accurate. No formatting issues. Perfect for financial statements.", avatar: "RP", accent: "from-blue-600 to-indigo-700" },
  ...
  { name: "Isabella Rossi", role: "Lawyer", country: "Italy", flag: "🇮🇹", rating: 5, text: "Privacy is non-negotiable for legal work. OnlyMyPDF's GDPR compliance and auto-deletion policy earned my trust.", avatar: "IR", accent: "from-red-600 to-rose-700" },
  ...
  { name: "Zara Nkosi", role: "NGO Coordinator", country: "South Africa", flag: "🇿🇦", rating: 5, text: "We work with limited internet. OnlyMyPDF loads fast, processes quickly, and compressed files are easy to share.", avatar: "ZN", accent: "from-amber-600 to-yellow-700" },
  ...
  { name: "Kofi Asante", role: "Business Owner", country: "Ghana", flag: "🇬🇭", rating: 5, text: "OnlyMyPDF helps me create professional invoices. The Word to PDF conversion is flawless — clients are impressed.", avatar: "KA", accent: "from-green-600 to-emerald-700" },
  ...
  { name: "Marco Bianchi", role: "Sales Director", country: "Italy", flag: "🇮🇹", rating: 5, text: "Our sales team sends proposals daily. OnlyMyPDF's merge + compress workflow means we never miss a deadline.", avatar: "MB", accent: "from-orange-600 to-red-700" },
  ...
  { name: "Thomas Anderson", role: "Consultant", country: "USA", flag: "🇺🇸", rating: 5, text: "I've tried every PDF tool out there. OnlyMyPDF strikes the perfect balance between simplicity and power.", avatar: "TA", accent: "from-slate-500 to-gray-700" },
  { name: "Sara García", role: "Translator", country: "Spain", flag: "🇪🇸", rating: 5, text: "Converting documents between formats while preserving layout is crucial. OnlyMyPDF handles it perfectly.", avatar: "SG", accent: "from-rose-600 to-pink-700" },
...
function CarouselRow({
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      >
        {doubled.map((t, i) => (
          <div key={`${t.name}-${i}`} className="w-[320px] shrink-0 sm:w-[340px]">
...
function NavArrow({
  ...
  return (
    <button
      ...
      className={cn(
        "absolute top-1/2 z-30 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full",
        "border border-gray-200/80 bg-white text-gray-700 shadow-md",
        "transition-all hover:border-pd-brand/30 hover:bg-pd-brand hover:text-white hover:shadow-lg",
        "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-pd-brand focus-visible:ring-offset-2",
...
function TestimonialsCarousel() {
  ...
  const applyTransform = useCallback(() => {
    const x = -halfWidthRef.current + offsetRef.current;
    const transform = `translate3d(${x}px, 0, 0)`;

    if (row1TrackRef.current) row1TrackRef.current.style.transform = transform;
    if (row2TrackRef.current) row2TrackRef.current.style.transform = transform;
  }, []);

  const measureLoopWidth = useCallback(() => {
    const track = row1TrackRef.current;
    if (!track) return;
  ...
  const scrollBy = useCallback(
    (direction: "left" | "right") => {
      const delta = direction === "right" ? MANUAL_SCROLL_STEP : -MANUAL_SCROLL_STEP;
  ...
  useEffect(() => {
    ...
    const tick = (timestamp: number) => {
      ...
      const half = halfWidthRef.current;
      const isHovered = hoverPausedRef.current;
      const isManualPaused = performance.now() < manualPauseUntilRef.current;

      if (half > 0 && !isHovered && !isManualPaused) {
  ...
  return (
    ...
    >
      <div className={cn("space-y-4 transition-opacity duration-300", ready ? "opacity-100" : "opacity-0")}>
        <CarouselRow items={ROW1} trackRef={row1TrackRef} />
        <CarouselRow items={ROW2} trackRef={row2TrackRef} />
      </div>

      <NavArrow direction="left" onClick={() => scrollBy("left")} />
      <NavArrow direction="right" onClick={() => scrollBy("right")} />
    </div>
  );
}
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-8 text-center">
            <div>
              <p className="text-2xl font-extrabold text-gray-900">
                4.9 <span className="text-base font-normal text-gray-400">/ 5</span>
              </p>
